...
/**
 * Express 앱(라우트)만 구성한다 — listen/cron 없음.
 * 상시 실행 서버(index.ts)와 서버리스 함수(api/index.ts)가 공유한다.
 * 토스/Notion 클라이언트와 캐시 싱글턴도 함께 노출(스케줄러가 재사용).
 */
export const toss = createTossClient(config);
export const quoteCache = new QuoteCache(toss, config.QUOTE_CACHE_TTL_MS);
export const notion = createNotionGateways(config);
